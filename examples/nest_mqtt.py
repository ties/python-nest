...
asset(NEST_CLIENT_ID and NEST_CLIENT_SECRET)
...
try:
    if nest.access_token is None:
        nest.request_token(os.environ.get('NEST_REQUEST_TOKEN', ''))
except:
    print(nest.authorize_url)
    exit()

#
# Use HBMQTT for mqtt w/ asyncio
#
async def publish_nest_events(host, port=1883):
    C = MQTTClient()
...
try:
    loop.run_until_complete(publish_nest_events(MQTT_HOST))
finally:
    loop.close()
